...
func update_calculations():
	...
	if profundidade > 0 and largura > 0 and Escala == 1:
		Y = altura + profundidade + 20
		X = profundidade + largura + 20
		
	if profundidade > 0 and largura > 0 and Escala == 2:
		
		Y = altura*2 + profundidade*2 + 20
		X = profundidade*2 + largura*2 + 20
